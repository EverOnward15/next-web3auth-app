...
export async function POST(req) {
  try {
    ...
    const privateKeyHex = body.privateKeyHex;

    if (!privateKeyHex || typeof privateKeyHex !== "string") {
      throw new Error("Missing or invalid privateKeyHex");
    }

    return new Response(JSON.stringify({ message: "Key received", key: privateKeyHex }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
